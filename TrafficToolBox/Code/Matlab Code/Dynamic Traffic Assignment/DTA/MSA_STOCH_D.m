function [flows] = MSA_STOCH_D(odmatrix,nodes,links,theta,alpha,beta,maxIt)
...
while it < maxIt && gap > 10^-8 
    ...
    figure(h) 
    hold on
    if(nrOD==1 && nnz(newOriginFlows)==1) %plot 'o' when only one route is used with only one OD pair
